                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="row float-right">
                                ...
                                <div class="">
                                    <a href="{{ route('dashboard.features.create') }}" class="btn btn-block btn-primary">
                        ...
                        <div class="collapse @if(count(request()->all())> 0) show @endif" id="collapseFilter">
                            <form method="GET" action="{{ route('dashboard.features.index') }}">
                                            ...
                                            <td>
                                                <ul class="list-inline">
                                                    <li class="list-inline-item">
                                                        <img alt="{{ $feature->name }}" class="table-avatar"
                                                            src="{{ $feature->image }}">
                                                    </li>
                                                </ul>
                                            </td>
                                            <td>
                                                <span class="badge @if ($feature->status === 'active') bg-success @else bg-danger @endif">
                                                    {{ $feature->status }}
                                                </span>
                                            </td>
                                            <td>{{ $feature->user->name }}</td>
                                            <td>{{ $feature->formattedCreatedAt }}</td>
                                            <td class="project-actions text-right">
                                                <form method="POST" action="{{ route('dashboard.features.destroy', $feature->id) }}">
                                                    @csrf
                                                    @method('DELETE')
